<template>
    <div id="Hot">
        <van-list v-model="loading" :finished="finished" finished-text="无更多电影了" @load="onLoad">
            <template>
                <div class="list_movies" v-for="(item,index) in list " :key="index">
            ...
            <div class="center_movies">
                ...
                <div>
                    <span>上映时间:{{ toYMD(item.premiereAt) }}</span>
                </div>
            </div>
            <div class="right_movies">
                <button v-if="item.isPresale">预购</button>
            </div>
...
export default {
    ...
    methods:{
        ...fromtime,
        onLoad() {
            console.log('Son-onload');
            this.loading=true;
            // this.finished = true
            // 我们发送请求，传入参数获取到相对应城市的id以及相对应的城市信息。
            // 想获取到具体的城市信息列表
            this.$axios('soon',{params:{pageNum:this.pageNum,cityId:this.$store.state.city.cityId}}).then(res=>{
                // 打印我们成功请求过来的数据
                console.log(res)
                // 判断当前的状态码
